...
**8.6 Row-level security.** 💡 RECOMMENDED as **defence in depth, not the primary
mechanism.** Primary authorization is the application authz service (§11) because the
permission model is richer than RLS expresses comfortably. Enable RLS with deny-by-default
on all tables; the app connects as a role that carries explicit grants; Supabase anon/authed
client keys get read access only to data safe for direct client reads (ideally none). The
consequence: **never expose the Supabase client to the browser for business data** — go
through Server Actions and route handlers.
