...
planned `erp.InvoiceCreated`, `erp.InvoiceIssued`, `erp.PaymentCompleted`), `erp.APBillPosted`,
`erp.APPaymentPosted` and the other AP events of ADR-033,
...
`iam.TokenIssued`, `iam.TokenRevoked`, `iam.UserPasswordSet` (ADR-034, ids only).
...
Phase 6  ERP finance core — 🟡 ERP Phase 1 "finance foundation" built (chart of accounts, periods,
         ...
         invoices, approval, receipts, allocation, balances, aging, §28); ERP Phase 3 accounts payable
         built (vendors, bills, input VAT, payments with withholding, aging, §28)
...
and stage entries cannot be deleted one by one. (4) Each service checks that the caller may read the record (otherwise 404) and then holds the delete permission for the record's scope target (otherwise 403); it marks the record with a
...
**ADR-033 — ERP Phase 3: accounts payable, with vendors owned by ERP, recoverable input VAT and tax withheld at
payment.** _Context:_ the owner asked for payables next: a supplier record, bills with VAT, approval, posting, payment
and what is owed. Four decisions were taken first — vendors are ERP records with an optional CRM link (a supplier is
often not a customer, and CRM's account is a sales record); input VAT is recoverable, to an input tax account named on
each tax rate; Egyptian withholding tax is deducted at payment, not at billing; and payments are approved under the
same kind of rule as bills, as a second, separate maker–checker. _Decision:_ (1) Migration
`20260917100000_erp_accounts_payable` adds seven `erp` tables — `ap_settings` (one row), `withholding_tax_rates`,
`vendors`, `ap_bills`, `ap_bill_lines`, `ap_payments`, `ap_payment_lines` — plus `tax_rates.input_tax_account_id`, the
AP_BILL and AP_PAYMENT number series types, and `ap_bill`/`ap_payment` branches in `journal_entries_source_check`. A
vendor holds its own name, tax registration, contact details, payment terms and default accounts, with
`crm_account_id` as an optional plain column (§8.3); nothing is copied from CRM. Live-row unique indexes refuse a
duplicate vendor name (case-insensitive, trimmed) or tax registration, and a second uncancelled bill with the same
supplier invoice number for one vendor. (2) Bill lines are priced exactly like invoice lines (ADR-023) but charge an
EXPENSE or ASSET account, and their VAT is debited to the rate's input tax account: a rate without one cannot be used
on a bill, so VAT that is not recoverable is simply not offered. Posting books Dr expense or asset per line and cost
centre, Dr input VAT, Cr payable. (3) Withholding is a payment-time deduction: a payment line names a bill, an amount
and an optional withholding rate; the base is the VAT-exclusive share of that amount — round(amount x (bill net /
bill total)) — and the tax withheld is round(base x rate), rounded half away from zero in both the pure rules
(`domain/ap.ts`) and the SQL CHECKs, so the database re-derives every figure. Cash paid is amount minus withheld.
Posting books Dr payable (the amount settled), Cr bank (cash) and Cr each withholding rate's payable account, so the
supplier's debt is cleared in full while the difference is owed to the tax authority. (4) Lifecycle mirrors AR for both
documents: DRAFT -> PENDING_APPROVAL -> APPROVED -> POSTED (bills then PARTIALLY_PAID/PAID), or CANCELLED. `ap_settings`
carries separate `bill_approval_required`/threshold and `payment_approval_required`/threshold plus one
`allow_self_approval`, off by default, so nobody approves what they created or submitted. (5) The database owns
settlement: an `ap_payments_apply` trigger moves each bill's `paid_minor` when a payment is posted or voided, locking
bills in id order (so simultaneous payments serialise and cannot overpay), and deferred checks at commit require a
bill's paid amount to equal the sum of its posted payment lines, a payment's totals to equal the sum of its lines, each
line's withholding base and amount to match the formula and the bill it names to belong to the payment's vendor. (6)
Corrections are voids: an unpaid posted bill or any posted payment is reversed by an equal and opposite journal entry
and marked cancelled; voiding a payment makes its bills owe again, so a paid bill cannot be voided until its payments
are. (7) Permissions follow AR: `erp.ap_vendor.{read,create,update}`,
`erp.ap_bill.{read,create,update,approve,post,cancel}`, `erp.ap_payment.{read,create,update,approve,post,cancel}`,
`erp.ap_aging.read` and `erp.ap_settings.administer`, all organisation-wide (ADR-025). No new IAM action, so no IAM
migration. `finance-admin` holds all of them, `accountant` enters and posts but does not approve, cancel or configure,
and the `erp-user` section role reads. Listing active tax rates and payment methods now also passes for a holder of
`erp.ap_bill.read` or `erp.ap_payment.read`, since the bill and payment forms need them without any receivables right.
(8) Events carry ids and amounts only: `erp.APBillApproved`, `APBillPosted`, `APBillCancelled`, `APPaymentApproved`,
`APPaymentPosted`, `APPaymentCancelled`. (9) The chart gains 1300 Input VAT Recoverable and 2200 Withholding Tax
Payable, and the seed creates the AP settings row pointing at 2100; no withholding rate is seeded, because rates are
the organisation's to enter. _Consequences:_ a service bug cannot overpay a bill, mis-withhold, or leave a document out
of step with its ledger entry — integration tests prove it by writing straight at the tables; the SQL and the Prisma
schema must change together; goods receipt and three-way matching belong to the later procurement slice, so a bill is
entered from the supplier's invoice alone; and withholding is deducted, recorded and owed, but no tax-authority return
or certificate is produced. _Revisit when:_ procurement ships (match bills to purchase orders and receipts), foreign
currency is required, or withholding certificates must be filed.

**ADR-034 — Passwords: people change their own; an administrator may set only a temporary one.** _Context:_ the
owner asked that users can change their own password and administrators can change users' passwords. Until now
nobody could change a password from inside TechVault: ADR-019 (7) let an administrator only email a reset link, and
said no administrator sets or sees a password. An administrator who knows someone's password indefinitely can act as
them, which breaks attribution (§2, §18.6). The owner chose "temporary, must change": an administrator may set a
password, but its holder must replace it before doing anything else. This amends ADR-019 (7). _Decision:_ (1) Migration
`20260918100000_iam_must_change_password` adds `iam.users.must_change_password` (boolean, default false). IAM reads it
on every request with `is_active`: the pure rule `sessionAccess` (`platform/auth/access.ts`) gives `disabled`,
`password-change` or `full`. For `password-change`, `requireUser` redirects every page to `/auth/change-password` and
`getActor` refuses every Server Action and route handler, so the one chokepoint covers the whole app; only
`getPasswordChangeActor`, used by the change-password action alone, admits such a session. (2) Changing one's own
password — `/account` ("My account", from the user menu) or the forced page — needs the current password, proved by
Supabase Auth in `identity-admin.changePasswordWithCurrent`: a throwaway client signs in with the current password,
sets the new one on that fresh session (which also meets Supabase's recent-sign-in rule), and signs the account out
of every session with `scope: "global"`; the action then clears this browser's cookies and redirects to sign-in with a
fixed message. No permission is involved: the proof is the password, and the service still refuses an inactive or
deleted account. (3) An administrator sets a temporary password from the user's Actions menu with
`iam.user.administer` on that user (a user outside their scope is a 404, as elsewhere), never for themselves (they use
My account) and never for an inactive account. The hold is written first, then Supabase's admin API sets the
password; if Supabase refuses, the hold is lifted again and the old password keeps working. It is audited at WARNING
and publishes `iam.UserPasswordSet`. Creating a user with a temporary password (ADR-019) now sets the hold as well, so
the create dialog's promise that "they change it after signing in" is enforced. (4) New passwords are 12 to 72
characters (bcrypt reads 72 bytes) and confirmed; a new password must differ from the current one; Supabase's own
policy (`weak_password`, `same_password`) is reported against the field. No password is ever stored, logged, audited
or put in an event — the audit records only that a password changed, and whether it replaced a temporary one.
_Consequences:_ an administrator never knows the password an account ends up with. Supabase's admin API cannot end
another user's sessions, so when an administrator sets a password the account's existing sessions are not signed out
at the provider; they are held by IAM instead — every one of them can only reach the change-password page, which
still demands the temporary password — and all of them end when the new password is chosen. Setting a password needs
`SUPABASE_SECRET_KEY` (§29 #15); changing one's own needs only the publishable key. The migration must run before the
code is deployed, because IAM selects the column on every request. _Revisit when:_ SSO replaces passwords, MFA is
added (re-authentication should then use the second factor), or Supabase gains a revoke-sessions-by-user admin call.

**ADR-035 — People sign in with a username; an email address is optional.** _Context:_ the owner asked to sign in
with "just user name and password", because staff have no email addresses yet. Supabase Auth signs people in by email
address only, and its Email provider is also the switch for every password sign-in: turning it off (as happened on
2026-09-18, recorded in `iam.login_history` as "Email logins are disabled") stops all sign-in, not just email.
_Decision:_ (1) Migration `20260918110000_iam_usernames` adds `iam.users.username` — lower case, 3 to 32 letters,
digits, dots, hyphens or underscores (CHECK `users_username_format`), unique among all users, deleted ones included, so
a username always points at one person in the audit trail. (2) A person with no mailbox gets an internal sign-in address,
`<username>@users.techvault.internal` (`platform/iam/usernames.ts`); `.internal` is reserved by ICANN for private use, so
no mail can be delivered to it, and accounts are created by an administrator with a password, so Supabase never mails
it either. `iam.users.email` keeps holding the sign-in address; `hasMailbox` / `visibleEmail` decide whether it is shown.
(3) Sign-in takes a username, or an email address for accounts that have one: `resolveSignInEmail` maps a username to
its account's sign-in address and an unknown one to the internal address it would have had, so Supabase refuses it
exactly like a wrong password and the page never reveals which usernames exist (§11.6). (4) Creating a user requires a
username; the email is optional, and without one the only setup is a temporary password (ADR-034) — an invitation is an
email. An address in the internal domain is refused as an email. (5) Changing a username is an administer-level act, like
changing the sign-in email: it is audited at CRITICAL, and for someone without a mailbox the sign-in address at Supabase
follows it. (6) The "Reset password" email is neither offered nor sent without a mailbox; the administrator sets a
temporary password instead. Deleting a user is confirmed by typing the username. (7) The admin list and details, the
header menu, My account, and the staff directory behind the Messenger and CRM people pickers show the username (or a real
email), never an internal address; the directory search matches usernames. Existing accounts keep signing in with their
email until an administrator gives them a username. _Consequences:_ nobody needs a mailbox to use TechVault, at the cost
of self-service password recovery for those without one — an administrator must set a temporary password. The Email
provider in Supabase must stay enabled. _Revisit when:_ staff get mailboxes (add their addresses; the username keeps
working) or SSO replaces passwords.
...
| Admin screens             | Users (full administration: add, edit, roles, unit, activate/deactivate, password reset, temporary password that must be replaced at next sign-in (ADR-034), soft delete, details — ADR-019), Roles, Permissions, Organisation units, Audit trail. Every user has My account (`/account`) to change their own password with the current one; a temporary password holds the account on `/auth/change-password` until replaced. People sign in with a username, and an email address is optional (ADR-035). Migrations `20260918100000_iam_must_change_password` and `20260918110000_iam_usernames` are applied to the local test database and, with the owner's approval, to Supabase on 2026-09-18 (the `must_change_password` column was already present there with the migration's exact definition, so that migration was recorded with `prisma migrate resolve --applied`); verified by direct query: both columns, the `users_username_format` CHECK and the partial unique index `users_username_unique`, 0 of 5 users held or named. On any database, migrate before deploying this code |
...
### 🟡 ERP — Phase 3 accounts payable, built 2026-09-17 at the owner's request

Design decisions are in ADR-033. Migration `20260917100000_erp_accounts_payable` is applied to the local test database
and, with the owner's approval after a walk-through of the SQL, to Supabase on 2026-09-18. Verified there by direct
query: the 7 AP tables with RLS enabled, 42 CHECK constraints, the 9 AP triggers plus the replaced
`journal_entries_source_check`, the 3 live-row unique indexes, the nullable `tax_rates.input_tax_account_id`, the
`number_series_document_type` CHECK widened to five document types, no privileges for `anon` or `authenticated`, and 0
rows in every AP table. The code was then deployed to production (`d33306d`, Vercel reported success;
`/api/health/ready` 200 with `database: ok`, and `/erp/finance/bills` redirects anonymous visitors to sign-in) and
`npm run db:seed` run on Supabase: 125 permissions, accounts 1300 Input VAT Recoverable and 2200 Withholding Tax Payable
created, AP settings on payable account 2100 (both approvals required, no threshold, self-approval off, 30-day terms,
30/60/90/120 buckets) and the BILL and PAY series — `finance-admin` and `platform-admin` hold all 17 AP permissions,
`accountant` 12 (not approve, cancel or settings) and `erp-user` 4 reads, each verified by direct query. Order on any
database: migrate, deploy the code, then seed.

| Area        | State                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Data model  | ✅ 7 `erp` tables: ap_settings, withholding_tax_rates, vendors, ap_bills, ap_bill_lines, ap_payments, ap_payment_lines, plus `tax_rates.input_tax_account_id`. Pricing and withholding CHECKs, guard triggers freezing posted documents, the settlement trigger and deferred reconciliation checks; live-row unique indexes on vendor name, tax registration and a vendor's supplier invoice number; deny-by-default RLS, API roles revoked.                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Services    | ✅ Vendors (list with balances, search, detail with aging and recent documents, create, edit, deactivate), bills (server-priced drafts, submit, approve under the AP settings rule, reject, post into a balanced linked journal, void by reversal, delete draft), payments (open bills for a vendor, drafts that count drafts already in progress against a bill, submit, approve, reject, post with withholding, void), AP aging, AP settings and withholding rates. Every write: permission → Zod → one transaction with audit record and outbox event. Period close and year-end close refuse unposted bills and payments.                                                                                                                                                                                                                                                                        |
| UI          | ✅ Vendors (list with status filter and balances; detail with owed, overdue, billed, settled, aging, recent bills and payments; form), Bills (list with status and overdue filters; line-entry form with input VAT preview; detail with lines, totals, settlements, approval trail and actions), Payments (list; form listing the vendor's open bills with amount, withholding rate and a live withheld/paid-out preview; detail with the per-bill breakdown), AP aging, AP settings (approval rules for bills and payments, defaults, aging buckets, withholding rates, BILL and PAY numbering). AR settings now sets each tax rate's input VAT account. A bill's or payment's journal entry links back to it.                                                                                                                                                                                      |
| Tests       | ✅ 10 unit tests (withholding base and rounding, payment totals, schemas, roles) and 23 integration tests (server pricing, duplicate supplier invoice, inactive vendor, a tax rate without an input account refused, asset lines, self-approval and thresholds, the balanced posting Dr expense/Dr input VAT/Cr payable, posted immutability straight at the tables, withholding on the VAT-exclusive part, partial payment counting drafts in progress, another vendor's bill refused, two approved payments for the whole bill where only one posts, voids restoring what is owed, a withholding base forced past the formula refused at commit, vendor balance and AP aging tied to the payable account in the ledger, period close, and allowed/refused per role including a payables-only clerk). `npm run verify` 275 passing; full integration suite 216 passing; build succeeds (84 routes). |
| **Missing** | Production has no vendor, withholding rate, tax rate with an input VAT account, or accounting period yet, so nothing can be entered or posted until a finance administrator adds them (§29 #25, #45). No signed-in browser walk-through and no Playwright spec (§29 #42). No withholding tax certificates or returns, and no purchase orders or goods receipts, so a bill is entered from the supplier's invoice alone (§29 #43, #44). AP lists and aging aggregate transactional tables live, like AR (§29 #35).                                                                                                                                                                                                                                                                                                                                                                                    |

### 📋 Not started

ECM, HRIS and BI, and ERP beyond finance, receivables and payables. The ECM, HRIS and BI navigation entries and permission keys are declared
...
| 42  | **Payables not exercised in a browser or in Playwright**           | The AP services and database rules are covered by 23 integration tests, but the vendor, bill, payment, aging and settings screens have not been walked through signed in, and there is no `erp-ap` e2e spec                                                                                                             | Walk vendor → bill → approve → post → pay → void on a non-production database; add the spec with the other ERP journeys                                                                               | 🟡 Medium |
| 43  | **Withholding is deducted but not reported**                       | Payments withhold tax and credit it to a liability account (ADR-033), but no Egyptian withholding certificate or quarterly return is produced                                                                                                                                                                           | Add a withholding report and certificate when the filing is needed, or export the liability account's activity                                                                                        | 🟡 Medium |
| 44  | **Bills are entered without a purchase order or goods receipt**    | There is no three-way match: a bill is taken from the supplier's invoice alone, so nothing checks it against what was ordered and received                                                                                                                                                                              | Build the procurement slice (purchase orders, goods receipts, accrual) and match bills against it                                                                                                     | 🟡 Medium |
| 45  | **Payables need configuration before first use**                   | Production has the payables schema and settings but no withholding tax rates, and no tax rate carries an input VAT account, so a bill cannot charge recoverable VAT and no tax can be withheld. Both approvals are on with self-approval off, so one person alone cannot take a bill or payment through (as in AR, #36) | A finance administrator adds the withholding rates in AP settings and the input VAT account on each tax rate in AR settings, and either grants approval to a second person or relaxes the AP settings | 🟡 Medium |
| 46  | **Password changes not exercised in a browser**                    | Setting a temporary password, the forced change, and changing one's own password are covered by 8 integration tests and 6 unit tests with Supabase Auth faked; the real Supabase calls (sign-in with the current password, update, global sign-out) and the redirects have not run against Supabase                     | After the migration is on Supabase, walk: admin sets a temporary password → user signs in → forced page → new password → signed out everywhere → signs in; then My account change                     | 🟡 Medium |
| 47  | **No password recovery without a mailbox**                         | People with no email address sign in by username (ADR-035), so "forgot password" has nowhere to send a link: only an administrator can let them back in, by setting a temporary password                                                                                                                                | Add their email addresses when they get mailboxes; the username keeps working                                                                                                                         | 🟢 Low    |
| 48  | **Username sign-in not exercised against Supabase**                | Covered by 7 integration and 4 unit tests with Supabase Auth faked; creating an account with an internal `.internal` address has not run against the real Supabase Auth, which could still reject the address                                                                                                           | After the migrations, create one test user without an email and sign in with its username                                                                                                             | 🟡 Medium |
