...
Design decisions are in ADR-033. Migration `20260917100000_erp_accounts_payable` is applied to the **local test
database only**; Supabase awaits the owner's schema review (§8.5). `prisma migrate diff` reports no drift. Order on any
database: migrate, deploy the code, then `npm run db:seed` (10 new permissions, the two new accounts, the AP settings
row and the BILL and PAY number series).
...
| 42  | **Payables not exercised in a browser or in Playwright**           | The AP services and database rules are covered by 23 integration tests, but the vendor, bill, payment, aging and settings screens have not been walked through signed in, and there is no `erp-ap` e2e spec                                                          | Apply the migration to a non-production database, seed, then walk vendor → bill → approve → post → pay → void; add the spec with the other ERP journeys | 🟡 Medium |
