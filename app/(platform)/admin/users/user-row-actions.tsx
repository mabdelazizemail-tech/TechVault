...
type DialogKind = "edit" | "roles" | "unit" | "status" | "reset" | "delete";
...
export function UserRowActions({
  ...
  return (
    <>
      ...
      <DropdownMenu.Root modal={false}>
        ...
        <DropdownMenu.Portal>
          ...
          >
            ...
            {rights.update && (
              <DropdownMenu.Item className={ITEM} onSelect={() => setDialog("edit")}>
                Edit
              </DropdownMenu.Item>
            )}
            {rights.administer && (
              <DropdownMenu.Item className={ITEM} onSelect={() => setDialog("roles")}>
                Change role
              </DropdownMenu.Item>
            )}
            {rights.update && (
              <DropdownMenu.Item className={ITEM} onSelect={() => setDialog("unit")}>
                Change organisation unit
              </DropdownMenu.Item>
            )}
            {rights.update && (
              <DropdownMenu.Item className={ITEM} onSelect={() => setDialog("status")}>
                {user.isActive ? "Deactivate" : "Activate"}
              </DropdownMenu.Item>
            )}
            {rights.administer && user.isActive && (
              <DropdownMenu.Item className={ITEM} onSelect={() => setDialog("reset")}>
                Reset password
              </DropdownMenu.Item>
            )}
            {rights.delete && (
      ...
      <EditUserDialog user={user} open={dialog === "edit"} onOpenChange={onOpenChange} />
      <RolesDialog user={user} open={dialog === "roles"} onOpenChange={onOpenChange} />
      <UnitDialog user={user} open={dialog === "unit"} onOpenChange={onOpenChange} />
      <StatusDialog user={user} open={dialog === "status"} onOpenChange={onOpenChange} />
      <ResetPasswordDialog
        user={user}
        open={dialog === "reset"}
